...
+ (NSString *)obtainFirstLetter:(NSString *)sourceStr
{
    //判断首字符是否为字母
    NSString *regex = @"[A-Za-z]+";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    NSString *initialStr = [sourceStr length]?[sourceStr substringToIndex:1]:@"";
    if ([predicate evaluateWithObject:initialStr])
    {
        //首字母大写
        return [initialStr capitalizedString];
    }
    else
    {
        NSString *pinYinResult = [NSString string];
        for(int j=0;j<sourceStr.length;j++)
        {
         NSString *singlePinyinLetter = [[NSString stringWithFormat:@"%c",
                                                 ...
                                                 pinyinFirstLetter([sourceStr characterAtIndex:j])]uppercaseString];
                //                    NSLog(@"singlePinyinLetter ==%@",singlePinyinLetter);
                
                pinYinResult = [pinYinResult stringByAppendingString:singlePinyinLetter];
        }
        return [NSString stringWithFormat:@"%c", [pinYinResult characterAtIndex:0]];
    }
